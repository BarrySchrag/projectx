...
videodir = "ava/videos"
annotfile = "ava/test_ava_train_v1.0.csv"
actionlistfile = "ava/ava_action_list_v1.0.pbtxt"
outdir = "ava"
...
def mkdir_p(path):
    try:
        _supermakedirs ( path, stat.S_IEXEC )
...
for ann in annots:
    ...
    # Miracle_1-zC5Fh2tTS1U
    # Greatest_Kung_Fu_movie_ever--5KQ66BBWC4
    print(videofile_noext)
    videofile = subprocess.check_output(['ls '+ videofile_noext],shell=True).decode("utf-8")[:-1]

    #videofile = subprocess.Popen(['ls ',videofile_noext], shell=True, stdout=subprocess.PIPE).communicate()[0]
    print(videofile)
    print( videofile.split ( '.' )[-1])
    #videofile = subprocess.call('ls ' + videofile_noext + '*', shell=True, check=True)
    ...
    print ( str ( middle_frame_timestamp ) )

    outdir_keyframe = outdir_midframe + '/' + video_id + '/'
    mkdir_p ( outdir_keyframe )
    # ffmpeg -ss 904.000000 -i ava/videos/Greatest_Kung_Fu_movie_ever--5KQ66BBWC4.mp4 -frames:v 1 ava/midframes/-5KQ66BBWC4/904.jpg
    ...
    #rm ava/midframes/-5KQ66BBWC4/916.jpg; ffmpeg -ss 916.000000 -i ava/videos/Greatest_Kung_Fu_movie_ever--5KQ66BBWC4.mp4 -frames:v 1 ava/midframes/-5KQ66BBWC4/916.jpg
    ...
    ffmpeg_command = 'rm %(outpath)s; ffmpeg -ss %(start_timestamp)s -i %(videopath)s -g 1 -force_key_frames 0 -t %(' \
                     'clip_length)d -strict -2 %(outpath)s' % {
    ...
    subprocess.call ( ffmpeg_command, shell=True )
    # subprocess.call('eog %(outpath)s &' % {'outpath': outpath}, shell=True)
# rm ava/clips/-5KQ66BBWC4/916.mp4; ffmpeg -ss 0:15:11 -i ava/videos/Greatest_Kung_Fu_movie_ever--5KQ66BBWC4.mp4 -g 1 -force_key_frames 0 -t 7 ava/clips/-5KQ66BBWC4/916.mp4
